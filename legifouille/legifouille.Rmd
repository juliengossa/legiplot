...
```{r load}
legifouille <- read.csv("code_education.csv", quote='"') %>%
...
explo <- function(date) {
  ...
  legifouille %>%
    ...
    slice_head(n = 1, by = c(lien_naturetexte, lien_numtexte)) %>%
    select(lien_naturetexte,lien_numtexte,lien_datesignatexte,lien_url) %>%
    spoiler_table("Textes modifiants (hors décrets)", trim = 10)
}
```
